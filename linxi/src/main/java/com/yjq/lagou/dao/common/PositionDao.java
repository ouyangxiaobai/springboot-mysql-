...
public interface PositionDao extends JpaRepository<Position, Long>{
	...
	@Query(value="select count(*) from position where state = :state and degree=:degree",nativeQuery=true)
	int getPositionTotalContent(@Param("state")String state,@Param("degree")String degree);
	...
	//nativeQuery=true:把hql语句转化为原生sql语句,因为这里hql语句中不支持limit
	@Query(value="select * from position where state = :state and degree= :degree order by create_time desc limit :offset,:pageSize",nativeQuery=true)
	public List<Position> findPositionListContent(@Param("state")String state,@Param("offset")int offset,@Param("pageSize")int pageSize,@Param("degree")String degree);
